...
(function ($) {
    ...
    $.extend($.NumberUtils, {
        ...
        percentFormat: function (cellVal) {
            return $.isNumeric(cellVal) ? $.NumberUtils.round(cellVal * 100,0) + "%" : "";
        },
        ...
        round: function (v1, precision) {
            ...
            if (!isNaN(v1)) {
                ...
                if (precision != null && !isNaN(Number(precision))) {
                    return Number(v1).toFixed(precision);
                } else {
                    return Number(v1);
                }
            } else {
        ...
        //去掉小数点末尾没用的0【本来用parseFloat(),但是这个在0.000000000000100的时候会出现科学计数法，不是本来想使用的预期】
