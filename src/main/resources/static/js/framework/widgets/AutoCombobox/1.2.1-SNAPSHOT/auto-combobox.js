...
(function ($, undefined) {
    ...
    $.extend($.autoCombo, {
        ...
        showDropDown: function (_isDropDownShow) {
            $(this).each(function () {
                ...
                if (_isDropDownShow) {
                    ...
                    //是否记忆数据 并且 不允许为空
                    if (p.view.isRememberValue === true && (p.view.isAllowEmpty === false || $this.val().length != 0)) {
		...
		selectFirstRow: function(){
			var _thisElm = this[0], p = _thisElm.p,
				rowIdAttrName = getDataGridRowIdAttributeName.call(_thisElm), rowNum = 0,
				$trElem = p._dataPanel.find("tr.selected[" + rowIdAttrName + "]");
    ...
    var
        ...
        _initial = function () {
            $(this).each(function () {
                ...
                //
                $this.off(p.event.actionEvents + p.id);
                $this.on(p.event.actionEvents + p.id, _onAction);
                //
                $this.off("keydown." + p.id);
                $this.on("keydown." + p.id, _onKeyDown);
                //
                $(window.document).off(windowEvent);
                ...
                //
                $this.off(p.event.lossEventsForSelf + p.id);
                $this.on(p.event.lossEventsForSelf + p.id, function () {
                    // 关闭下拉框的事件
                    _onLose.apply(_thisElm, arguments);
                });
                // 模板采用Underscore
         ...
         * 鼠标移入下拉
         */
        _onMouseOverDropDown = function () {
            //
            this.p._isMouseOut = false;
            //
            //event.stopPropagation();
        },
         ...
         * 鼠标离开下拉
         */
        _onMouseOutDropDown = function () {
            //
            this.p._isMouseOut = true;
            //
            //event.stopPropagation();
        },
        ...
        _createLoadingBar = function () {
            ...
            if (p._loadingBar && p._loadingBar.length > 0) {
                ...
                p._loadingBar = $('<div id="' + lbid + '" style="display: none;position: absolute;top: 0px;left: 0px;"><div class="auto-comboBox-loadingbar-img"></div></div>');
                ...
                if ($t.is('span') && $td.is('td') || typeof event === 'string') {
                    ...
                    if (tdId === getPrevPageBtnId.call(_thisElm)) {
                        ...
                        if (_currentPage > 1) {
                            ...
                            _isRefresh = true;
                            typeof event === 'string' ? _thisElm.isPrePageOfKey = true : delete _thisElm.isPrePageOfKey;
                        }
                    }
                    // 下一页按钮事件
                    if (tdId === getNextPageBtnId.call(_thisElm)) {
                        ...
                        if (_currentPage < p.async.requestData[p.async.pageFieldDefine.totalPage]) {
                            _currentPage = _currentPage + 1;
                            _isRefresh = true;
                            delete _thisElm.isPrePageOfKey;
                        }
                    }
                    // 末页按钮事件
        ...
        _buildDataGridHtmlTemplate = function () {
            ...
            //---------构造 表体(数据网格)所在的div
            _tableTemplate += '<div style="overflow-y:auto;overflow-x:hidden">';
            //有数据
            ...
            for (i = 0; i < cms.length; i++) {
                ...
                if (cm.isHide === true) {
                    ...
                    if(cm.wordBreak){
						tdStyle += 'word-break:' + cm.wordBreak+ ';';
					}
                    tdStyle += '"';
        ...
        _resizeColWidth = function (totalWidth) {
            ...
            for (i = 0; i < view.colModels.length; i++) {
                ...
                if (colModel.isHide !== true) {
                    //列宽 = 列宽比例 * (总列宽-滚动条宽度)
                    colModel.width = colModel._widthRate * totalWidth;
                    $('[colName="' + colModel.name + '"]', p._dropDown).outerWidth(colModel.width);
                }
            }
        },
         ...
         * 显示的字段的宽度比率 colModel._widthRate
        ...
        _calculateInfo = function () {
            ...
            for (i = 0; i < p.view.colModels.length; i++) {
                ...
                if (colModel.isHide !== true) {
                    //列宽比例 = (配置列宽/总列宽)
                    colModel._widthRate = (colModel.width / p._totalColWidth);
                }
            }
            //高度
            if (p.view.height == 0) {
